"""v5 (블랙포인트 + NR 강화) 시안 — 사용자가 보던 리프트 컷으로."""
import json
import math
import sys
...
for name in ["RY901702.ARW", "RY901493.ARW"]:
    ...
    r = export_jpeg(row["path"], out / f"v5_{name}.jpg", 6.0,
                    lift_ev=lift, mid_gamma=gamma, nr_level=nr)
    print(name, "lift=", round(lift, 2), "bp=", round(min(0.10, 0.035 * lift), 3),
